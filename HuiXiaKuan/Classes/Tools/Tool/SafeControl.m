...
- (BOOL)safeWriteToPlistWithPath:(NSString*)path
{
    ...
    LQLog(NSLocalizedString(@"写入DIC文件:%@", nil),path);
    return [data writeToFile:path atomically:YES];
}

+ (instancetype)safeReadFromPlistWithPath:(NSString*)path
{
    LQLog(NSLocalizedString(@"读取DIC文件:%@", nil),path);
...
- (BOOL)safeWriteToPlistWithPath:(NSString*)path
{
     LQLog(NSLocalizedString(@"写入Array文件:%@", nil),path);
    NSData * data = [NSKeyedArchiver archivedDataWithRootObject:self];
    return [data writeToFile:path atomically:YES];
}
...
+ (instancetype)safeReadFromPlistWithPath:(NSString*)path
{
     LQLog(NSLocalizedString(@"读取Array文件:%@", nil),path);
    NSData * data = [NSData dataWithContentsOfFile:path];
    return  [NSKeyedUnarchiver unarchiveObjectWithData:data];
}
@end
