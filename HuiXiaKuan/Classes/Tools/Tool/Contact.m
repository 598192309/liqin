...
//        [NVMContactManager postMainThreadNotification:NVMContactAccessFailedNotification];
        [[NSNotificationCenter defaultCenter] postNotificationName:NVMContactAccessFailedNotification object:nil];
...
//                }
        //做一个友好的提示
//        UIAlertView * alart = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"温馨提示", nil) message:NSLocalizedString(@"请您设置允许APP访问您的通讯录\nSettings>General>Privacy", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"确定", nil) otherButtonTitles:nil, nil];
//        [alart show];
    }
}
...
//        NSString *fullName = [NSString stringWithFormat:@"%@%@%@",firstName,middlename,lastName];
        ...
        [dic setValue:fullName forKey:@"full_name"];
...
#warning todo传递数据
    //不管下面的上传
    if (self.ContactUploadSucBlock) {
        self.ContactUploadSucBlock(@"success");
    }
