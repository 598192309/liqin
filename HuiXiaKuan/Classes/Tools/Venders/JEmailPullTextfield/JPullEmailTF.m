...
//    [self setUpEmailSuffixArray];
    ...
    self.pullTableView.tableFooterView = [UIView new];
...
-(void)setMailsuffixData:(NSArray *)mailsuffixData{
    ...
    _matchedSuffixArray = mailsuffixData;
    self.pullTableView.hidden = NO;
    [self.pullTableView reloadData];
}
...
//    cell.emailLabel.text = [formerStr stringByAppendingString:self.matchedSuffixArray[indexPath.row]];
    cell.emailLabel.text = self.matchedSuffixArray[indexPath.row];
...
//    self.text = [formerStr stringByAppendingString:self.matchedSuffixArray[sender.tag]];
    self.text = self.matchedSuffixArray[sender.tag];
    self.pullTableView.hidden = YES;
    
    [self resignFirstResponder];
    if ([self.JPullEmailTFDelegate respondsToSelector:@selector(JPullEmailTFTapCellDelegate:title:)]) {
        [self.JPullEmailTFDelegate JPullEmailTFTapCellDelegate:sender.tag title:self.matchedSuffixArray[sender.tag]];
    }
}
